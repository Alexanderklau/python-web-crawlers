...
class WeiboLogin(object):
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:45.0) Gecko/20100101 Firefox/45.0"})
    ...
    def login(self,user_name,pass_word):
        ...
        if json_data["showpin"] == 1:
            ...
            with open("captcha.jpeg", "wb") as file_out: file_out.write(self.session.get(url).content)
            code = input("请输入验证码:")
            post_data["pcid"] = json_data["pcid"]
            post_data["door"] = code
            #http://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.18)&_=1481187311152
            ...
            if json_data_1["retcode"] == "0":
                params = {"callback": "sinaSSOController.callbackLoginStatus",
                           "client": "ssologin.js(v1.4.18)",
                           "ticket": json_data_1["ticket"],
                           "ssosavestate": int(time.time()), "_": int(time.time() * 1000),}
