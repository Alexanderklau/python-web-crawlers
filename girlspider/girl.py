...
class mzitu():
    def all_url(self, url):
        ...
        for a in all_a:
            ...
            self.mkdir('/home/lau/PycharmProjects/python-web-crawlers/girlspider/' + path)
            os.chdir(path)  ##切换到目录
            href = a['href']
    ...
    def mkdir(self, path):  ##这个函数创建文件夹
        path = path.strip()
        isExists = os.path.exists(os.path.join("/home/lau/PycharmProjects/python-web-crawlers/girlspider/down", path))
        if not isExists:
            print(u'建了一个名字叫做', path, u'的文件夹！')
            os.makedirs(os.path.join("/home/lau/PycharmProjects/python-web-crawlers/girlspider/down", path))
            return True
        else:
